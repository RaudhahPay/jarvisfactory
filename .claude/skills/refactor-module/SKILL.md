...
Migrate the module/component in `(the user's input / arguments for this command)` from its current implementation (old dependency, API, or pattern) to the target one — behavior-preserving, test-first, one unit at a time.
