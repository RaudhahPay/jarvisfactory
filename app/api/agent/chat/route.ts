// ============================================================
// JARVISFACTORY v2 — Phase A: Chat mode (streaming, persisted)
// ============================================================
...
// getAuthedDb(); quota enforced before the call.
// ============================================================
...
export async function POST(req: NextRequest) {
  ...
  if (!conversationId) {
    const { data: conv, error } = await db
      ...
      .select('id')
      .single()
    if (error || !conv) return Response.json({ error: `Could not start conversation: ${error?.message || 'unknown'}` }, { status: 500 })
