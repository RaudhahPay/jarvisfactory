...
${initialPrompt ? '<strong style="color:#00e5b0">Your idea is ready in the prompt box on the left — review it and click ⚡ Launch when ready.</strong>' : 'Tell me what you want to build.'}`)
  ...
  async function callClaudeOnce(sys: string, msg: string, maxTok: number, useAttachments: boolean, model?: string, imageDataUrl?: string): Promise<string> {
    ...
    const r = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ model: model || 'claude-sonnet-4-6', max_tokens: maxTok, system: sys, messages }),
      keepalive: false,
    })

    let data: any
    try { data = await r.json() }
    catch { throw new Error(`Server returned non-JSON (${r.status} ${r.statusText})`) }
  ...
  async function callClaude(sys: string, msg: string, maxTok = 10000, useAttachments = false, model?: string, imageDataUrl?: string) {
    ...
    for(let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++){
      try {
        ...
        lastErr = err
        const isNetwork = /Failed to fetch|NetworkError|TypeError: Load failed|network error|stream|ECONN|ETIMED|read.*ECONNRESET|fetch failed|getaddrinfo|EAI_AGAIN|socket hang up/i.test(err?.message || '')
        const retriable = isNetwork || err?.transient === true
        if(!retriable || attempt === MAX_ATTEMPTS) break
        const delayMs = 1500 * attempt // 1.5s, then 3s
        addLog(`Transient error: ${err?.message?.slice(0,80)} — retrying in ${(delayMs/1000).toFixed(1)}s (${attempt}/${MAX_ATTEMPTS-1})...`, 'warn')
  ...
  async function callClaudeAgentic(params: { system: string; messages: any[]; tools: any[]; max_tokens: number; model?: string }) {
    ...
    for(let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++){
      try {
        ...
        let data: any
        try { data = await r.json() } catch { throw new Error(`Server returned non-JSON (${r.status})`) }
        if(!r.ok){
          const e: any = new Error(data?.error?.message || `API error ${r.status}`)
          e.transient = r.status >= 500 || r.status === 429
          throw e
        ...
        addLog(`Agentic call transient error: ${err?.message?.slice(0,80)} — retrying in ${(delayMs/1000).toFixed(1)}s...`, 'warn')
        await new Promise(res => setTimeout(res, delayMs))
      }
    }
...
- If integrations were listed, reflect them in 'tech_stack.integrations'`
  ...
  async function approveBuild() {
    ...
    try {
      ...
      const lessons = await loadJarvisLessons(jarvis?.id || null, 30)
      ...
      setAgentStatus(s => ({ ...s, architect: { status: 'working' }, designer: { status: 'working' } }))
      ...
      if(archResult.status === 'fulfilled'){
        try {
          ...
          setAgentStatus(s => ({ ...s, architect: { status: 'done', note: `${componentCount} screens · ${moduleCount} modules` } }))
          ...
          setAgentStatus(s => ({ ...s, architect: { status: 'failed', note: err.message } }))
        ...
        setAgentStatus(s => ({ ...s, architect: { status: 'failed', note: archResult.reason?.message } }))
      ...
      if(designResult.status === 'fulfilled'){
        try {
          ...
          setAgentStatus(s => ({ ...s, designer: { status: 'done', note: `${paletteCount} colors · ${componentCount2} component patterns` } }))
          ...
          setAgentStatus(s => ({ ...s, designer: { status: 'failed', note: err.message } }))
          addLog(`${DESIGNER.emoji} Designer parse failed: ${err.message}`, 'err')
          designSystem = null
        }
      } else {
        setAgentStatus(s => ({ ...s, designer: { status: 'failed', note: designResult.reason?.message } }))
      ...
      setAgentStatus(s => ({ ...s, builder: { status: 'working', note: 'Starting...' } }))
      ...
      const isSimpleApp = (complexityLower === 'simple' || complexityLower === 'low')
        && featureCount <= 6
        && buildModules.length <= 4
      ...
      if(useModular){
        ...
        for(let mi = 0; mi < buildModules.length; mi++){
          ...
          setAgentStatus(s => ({ ...s, builder: { status: 'working', note: `Module ${moduleNum}/${buildModules.length}: ${mod.name}` } }))
        ...
        setAgentStatus(s => ({ ...s, builder: { status: 'working', note: 'Integration pass: auditing & fixing cross-module bugs' } }))
        ...
        const builderState = await runBuilderAgent({
          ...
          onProgress: (e) => {
            if(e.toolName){
              ...
              setAgentStatus(s => ({ ...s, builder: { status: 'working', note: `iter ${e.iteration}: ${verb}` } }))
            } else if(e.thinking){
      ...
      for(let pass = 1; pass <= MAX_BUILD_PASSES; pass++){
        ...
        if(pass === 1){
          setAgentStatus(s => ({ ...s, builder: { status: 'done', note: `${(code.length/4).toFixed(0)} tokens` } }))
          setAgentStatus(s => ({ ...s, qa: { status: 'working' } }))
        ...
        try {
          ...
          const screenshot = await captureAppScreenshot(code)
          if(screenshot){
        ...
        if(!blocked){
          ...
          setAgentStatus(s => ({ ...s, qa: { status: 'done', note: `Score: ${score}/100${certified ? ' · Certified' : ' · Issues remain'}` } }))
          if(pass > 1){
            addLog(`${BUILDER.emoji} Builder pass ${pass} complete (after QA fixes).`, 'ok')
            setAgentStatus(s => ({ ...s, builder: { status: 'done', note: `${pass} passes` } }))
          }
        ...
        setAgentStatus(s => ({ ...s, builder: { status: 'working', note: `Pass ${pass+1} (fixing ${criticalCount} issue${criticalCount===1?'':'s'})` } }))
      ...
      if(user) {
        ...
        if(newRow) {
          ...
          await recordBuildOutcome({
            ...
            iterations: builderState.iterations,
            ...
            notes: `Builder iterations: ${builderState.iterations}, tool calls: ${builderState.toolCalls?.length || 0}`,
          })
